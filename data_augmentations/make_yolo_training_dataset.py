...
training10K_data_path = '/data/sswang/data/isc_data/subset/training_data_subset10K/'
background_img_path = '/data/sswang/data/isc_data/subset/training_data_background/'
...
# 保存增强之后的图片和yolo标签
def save_auged_file(folder_name, img_name ,yolo_label, final_img):
    base_path = os.path.join('/data/sswang/data/isc_data/subset/yolo_training_10K', folder_name)
    ...
    # 创建标签文件,存储标签信息
    yolo_label_path = os.path.join(output_label_path, img_name.split('.')[0] + '.txt')
...
for img in train_img_list:
    ...
    # 将贴图 贴在背景图上
    overlaied_img, yolo_label = overlay_image(bg_img=bg_img, overlay=overlay_img)
    ...
    if 0 <= counter < 7000:
        save_auged_file(folder_name = 'train', img_name = img ,yolo_label = yolo_label, final_img = final_img)

    # 7000--8999编号的图片被放在test文件夹中
    elif 7000<=counter < 9000:
        save_auged_file(folder_name = 'test', img_name = img ,yolo_label = yolo_label, final_img = final_img)

    # 9000--9999编号的图片放在valid文件夹中
    else:
        save_auged_file(folder_name = 'valid', img_name = img ,yolo_label = yolo_label, final_img = final_img)
