...
def get_random_screenshot_template():
    ...
    screenshot_list = [i for i in os.listdir(
        screenshot_path) if i.endswith('png')]
...
aug1 = imaugs.Compose(
    [
        ...
        ), caption_height=random.randint(20, 200), meme_bg_color=random_RGB(), p=0.6),
        ...
        imaugs.OverlayEmoji(emoji_path=get_ramdom_emoji(), opacity=random.uniform(0.5, 1), emoji_size=random.uniform(
            0, 1), x_pos=random.uniform(0.1, 0.7), y_pos=random.uniform(0.3, 0.7), p=1.0),
        imaugs.OverlayEmoji(emoji_path=get_ramdom_emoji(), opacity=random.uniform(0.5, 1), emoji_size=random.uniform(
            0, 1), x_pos=random.uniform(0.1, 0.5), y_pos=random.uniform(0.3, 0.7), p=0.6),
        imaugs.OverlayEmoji(emoji_path=get_ramdom_emoji(), opacity=random.uniform(0.2, 1), emoji_size=random.uniform(
            0, 1), x_pos=random.uniform(0.5, 0.7), y_pos=random.uniform(0.3, 0.7), p=0.3),


        # 在图像上叠加线段
        imaugs.OverlayStripes(line_width=random.uniform(0.3, 1), line_color=random_RGB(), line_angle=random.uniform(
            -180, 180), line_density=random.uniform(0.3, 1), line_type=get_line_type(), line_opacity=random.uniform(0.5, 1), p=0.6),

        # 在图片上叠加文字
        imaugs.OverlayText(text=get_random_overlaytext(), font_size=random.uniform(0.1, 0.3), opacity=random.uniform(
            0.3, 1), color=random_RGB(), x_pos=random.uniform(0.1, 0.6), y_pos=random.uniform(0.1, 0.7), p=0.6),

        # 把图片做成截图的样式
        imaugs.OverlayOntoScreenshot(template_filepath=get_random_screenshot_template(
        ), template_bboxes_filepath='C:/Users/Lenovo/Desktop/AI/yolo/data/screenshot_templates/bboxes.json', max_image_size_pixels=None, crop_src_to_fit=False, resize_src_to_match_template=True, p=0.6),


        # 给图片加上边框
        imaugs.Pad(w_factor=random.uniform(0.1, 0.3), h_factor=random.uniform(
            0.1, 0.3), color=random_RGB(), p=0.6),
        ...
        imaugs.PerspectiveTransform(sigma=random.randint(50, 150), dx=random.uniform(
            0, 10), dy=random.uniform(0, 10), seed=random.randint(1, 200), p=0.6),
        ...
        imaugs.RandomNoise(mean=random.random(), var=random.uniform(
            0.5, 1), seed=random.randint(10, 50),  p=0.6),
        ...
        # 打乱图片的像素分布（很费时间）
        imaugs.ShufflePixels(factor=random.uniform(
            0.3, 1), seed=random.randint(10, 100), p=0.6),

        # 让图片变倾斜
        imaugs.Skew(skew_factor=random.uniform(-2, 2),
                    axis=random.choice([0, 1]),  p=0.6),
